...
interface Notification {
  ...
  title: string;
  message: string;
  type: 'new_booking' | 'meeting_confirmed' | 'meeting_cancelled' | 'no_show';
  linkTo?: string;
  createdAt: Date;
  read: boolean;
  data?: any;
  ...
  useEffect(() => {
    ...
    const handleNewBooking = (data: any) => {
      addNotification({
        ...
        message: `New ${data.meetingType} counseling request received`,
        type: 'new_booking',
    ...
    const handleMeetingConfirmed = (data: any) => {
      addNotification({
        ...
        message: `Meeting confirmed by counselor for ${format(new Date(data.meetingDate), 'PPP')}`,
    ...
    const handleMeetingCancelled = (data: any) => {
      addNotification({
        ...
        message: `Meeting cancelled. Reason: ${data.cancellationReason || 'No reason provided'}`,
    ...
    const handleNoShow = (data: any) => {
      addNotification({
        title: 'No Show Reported',
        message: `No-show reported by ${data.reportedBy} for meeting on ${format(new Date(data.meetingDate), 'PPP')}`,
        type: 'no_show',
        linkTo: '/admin/meetings',
        data
      });
