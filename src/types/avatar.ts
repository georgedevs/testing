...
export const getAllAvatars = (): AvatarOption[] => {
  return Object.values(avatarOptions).flat();
};

export const getAvatarById = (id: string): AvatarOption | undefined => {
  return getAllAvatars().find(avatar => avatar.id === id);
};
...
export interface AvatarResponse {
  success: boolean;
  avatars: Record<AvatarCategory, AvatarOption[]>;
}
...
export interface AvatarApiError extends ApiResponse {
  error: string;
}